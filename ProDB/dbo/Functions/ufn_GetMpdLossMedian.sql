...
SELECT * FROM [dbo].[ufn_GetMpdLossMedian]('LE74796-W18')
SELECT * FROM [dbo].[ufn_GetMpdLossMedian]('LE74796-W19')
SELECT * FROM [dbo].[ufn_GetMpdLossMedian]('LE74796-W122')

Change Log:
2026-05-07 JC: Base ufn_Coral4p5_GetMpdLossMedian; Onchip_MPD_CHxx-->Onchip_loss_CHxx_MPD
*/
CREATE   FUNCTION [dbo].[ufn_GetMpdLossMedian]
(
    @LotWafer varchar(11)
)
...
BEGIN
    ...
    INSERT INTO @RawData(ChannelNo,MpdLoss)
        ...
        (
        VALUES
            (1, TRY_CONVERT(float, p.Onchip_loss_CH01_MPD)),
            (2, TRY_CONVERT(float, p.Onchip_loss_CH02_MPD)),
            (3, TRY_CONVERT(float, p.Onchip_loss_CH03_MPD)),
            (4, TRY_CONVERT(float, p.Onchip_loss_CH04_MPD)),
            (5, TRY_CONVERT(float, p.Onchip_loss_CH05_MPD)),
            (6, TRY_CONVERT(float, p.Onchip_loss_CH06_MPD)),
            (7, TRY_CONVERT(float, p.Onchip_loss_CH07_MPD)),
            (8, TRY_CONVERT(float, p.Onchip_loss_CH08_MPD))
        ) v(ChannelNo, MpdLoss)
