...
BEGIN
	...
	insert #ShippingUnit(Ship_date, Customer_Code, PO, Lot_Wafer_Box_ID, ProductFamily, LotWafer, ChipSN, ShippingBin)
    ...
    (
        select
            ...
            (
                case
                    when p.uec_onchip_low           > s.spec_uec_low
                     and p.uec_conchip_high         < s.spec_uec_high
                     and p.uec_onchip_std           < s.spec_std_multiplier
                     and p.onchip_loss_optical_low  > s.spec_loss_low
                     and p.onchip_loss_optical_high < s.spec_loss_high
                     and p.loss_range_high          < s.spec_loss_range_high
                     and p.ompd_range_high          < s.spec_ompd_range_high
                     and p.mpdm_mpds_dev            < s.spec_mpdm_mpds_dev
                     and p.ppi_low                  > s.spec_ppi_low
                     and p.ppi_high                 < s.spec_ppi_high
                     and p.heater_resistance_low    > s.spec_ht_low
                     and p.heater_resistance_high   < s.spec_ht_high
                     and p.dark_current_low         > s.spec_dc_low
                     and p.dark_current_high        < s.spec_dc_high
                     and p.onchip_loss_mpd_low      > s.spec_mpd_loss_low
                     and p.onchip_loss_mpd_high     < s.spec_mpd_loss_high
                     and p.mpd_loss_range_high      < s.spec_mpd_loss_range
                    then 1 else 0
                end
                as bit
            )
