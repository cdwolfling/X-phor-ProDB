...
*/
CREATE   VIEW [dbo].[XH_OUTTERCARTON]
AS
SELECT 
   DISTINCT f.Carton_ID_outter,
   f.Sumqty,
   e.Project,
   e.PO,
   e.Supplier_Code,
   ...
   e.pn,
   e.description1,
   e.PO + '-' + e.Supplier_Code + '-' + e.Material_PN + '-' + e.wekday + '-' + e.Lot_ID + '-' + e.Carton_ID_outter + '-' + CAST(f.Sumqty AS VARCHAR(15)) AS QR_code
FROM 
(
    SELECT  
        a.Project,
    ...
    FROM 
        Shipping_list a 
        CROSS APPLY dbo.ufn_GetLabelViewShipDate('XH_OUTTERCARTON') cfg
    LEFT JOIN 
...
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[XH_OUTTERCARTON] TO [Production1]
    AS [dbo];


GO
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[XH_OUTTERCARTON] TO [Production]
    AS [dbo];
