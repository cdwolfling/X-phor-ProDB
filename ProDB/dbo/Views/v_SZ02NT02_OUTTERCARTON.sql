...
*/
CREATE   VIEW [dbo].[v_SZ02NT02_OUTTERCARTON] AS
WITH RankedData AS (
    SELECT 
        /* 基础信息字段 */
        e.Project,                          
        ...
        /* 修改为按Carton_ID_Outter聚合的发货数量 */
        SUM(e.Ship_Qty) OVER (PARTITION BY e.Carton_ID_Outter) AS SHIP_QTY,             
        ...
        /* 修改后二维码生成字段 */
        (b.Material_PN + '&' + 
         CAST(SUM(e.Ship_Qty) OVER (PARTITION BY e.Carton_ID_Outter) AS VARCHAR) + '&' + 
        ...
        /* 供应商代码和Carton_ID_Outter的组合 */
        b.Supplier_Code + '.' + e.Carton_ID_Outter AS Supplier_Code_Carton_ID_Outter,
    ...
    FROM 
        Shipping_list e  
        CROSS APPLY dbo.ufn_GetLabelViewShipDate('v_SZ02NT02_OUTTERCARTON') cfg
...
SELECT 
    ...
    /* 供应商代码和Carton_ID_Outter的组合 */
    Supplier_Code_Carton_ID_Outter,
    
    /* 原始LOTID_WAFERID_BOXID */
    LOTID_WAFERID_BOXID_Original
    
FROM RankedData
WHERE rn = 1  -- 去重，只保留每个Carton_ID_Outter的最新记录
GO
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[v_SZ02NT02_OUTTERCARTON] TO [Production1]
    AS [dbo];


GO
GRANT VIEW DEFINITION
    ON OBJECT::[dbo].[v_SZ02NT02_OUTTERCARTON] TO [Production]
    AS [dbo];
